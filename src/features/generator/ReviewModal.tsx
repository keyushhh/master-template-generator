...
export function ReviewModal({ open, onClose, deck, ast, onPresent, onReorder, onToggleHidden }: ReviewModalProps) {
  ...
  const runExport = async (kind: 'pdf' | 'pptx') => {
    ...
    try {
      ...
      const fn = kind === 'pdf' ? mod.exportToPDF : mod.exportToPPTX;
      await fn(visibleIds, title, (current, total) => setProgress({ current, total }));
    } catch (err) {
  ...
  const btnBase =
    'h-[42px] px-5 flex items-center gap-2 text-[13px] font-bold rounded-[var(--radius-sharp)] transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed';

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-neutral-900/60 backdrop-blur-sm p-6" onClick={() => !busy && onClose()}>
      <div className="flex flex-col w-full max-w-5xl max-h-[90vh] bg-white rounded-[var(--radius-sharp)] shadow-2xl overflow-hidden" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="flex items-start justify-between gap-4 px-6 pt-5 pb-4 border-b border-neutral-150">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 bg-neutral-50">
          ...
          {coverage && (
            <div className={`mb-5 border rounded-[var(--radius-sharp)] px-4 py-3 ${hasWarnings ? 'border-amber-300 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}`}>
              ...
              {hasWarnings && (
                <ul className="mt-2 flex flex-col gap-1 text-[12px] text-amber-800/90 leading-relaxed list-disc pl-5">
                ...
                {deck.slides.map((slide) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <SlideStage slide={slide} ast={ast} num={num} scale={THUMB_SCALE} logoUrl={deck.logoUrl} />
                        ...
                        <button
                          ...
                          className={`absolute top-1.5 right-1.5 w-7 h-7 flex items-center justify-center rounded-[var(--radius-sharp)] shadow-sm transition-colors cursor-pointer ${
                        ...
                        {slide.hidden && (
                          <span className="absolute bottom-1.5 left-1.5 font-mono text-[9px] font-bold uppercase tracking-[0.1em] bg-neutral-900/80 text-white px-1.5 py-0.5 rounded-[var(--radius-sharp)]">Hidden</span>
                        )}
                      </div>
        ...
        {/* Footer actions */}
        <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-neutral-150">
          <div className="text-[12px] text-neutral-500 min-h-[18px]">
            {busy ? `Exporting ${busy.toUpperCase()} - slide ${progress.current} of ${progress.total}…` : error ? <span className="text-red-500">{error}</span> : 'Everything look right? Export or present.'}
          </div>
          <div className="flex items-center gap-2.5">
            <button onClick={onPresent} disabled={!!busy || visible.length === 0} className={`${btnBase} text-neutral-700 bg-white hover:bg-neutral-50 border border-neutral-200`}>
